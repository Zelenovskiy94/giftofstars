...
function scaleImage() {
		...
		document.addEventListener('touchstart', function (e) {
			...
			if(outputImage.contains(e.target)){
				outputImage.style.transform = 'scale(0.65) translateX(0px) translateY(210px)';
			}
	...
	for(let i = 0; i < btnColors.length; i++){
		document.addEventListener('click',function (e) {
			...
			if(e.target == btnColors[0]){
				...
				outputImage.className = 'output-image bg-white'
			...
			if(e.target == btnColors[1]){
				...
				if(!map.classList.contains("border-white") 
					&& btnView[0].classList.contains("view-map-elem-active")
					|| btnView[1].classList.contains("view-map-elem-active")) {
					map.classList.add('border-white')
				}
				outputImage.className = 'output-image bg-black'
			...
			if(e.target == btnColors[2]){
				...
				outputImage.className = 'output-image bg-blue'
				starmap.style.background = 'rgba(0,0,0,0.4)';
				border.style.borderColor = '#fff';
				border.classList.remove('starmap-black')
				colorEq = 'rgba(100,100,100,0.5)';
			...
			function setImageBg(i, cls) {
				if(e.target == btnColors[i]){
					btnColors[i].classList.toggle('btn-color-active');
					if(!map.classList.contains("border-white")
					&& btnView[0].classList.contains("view-map-elem-active")
					|| btnView[1].classList.contains("view-map-elem-active")) {
						map.classList.add('border-white')
					}
					outputImage.className = `output-image ${cls} bgImage`
					starmap.style.background = 'rgba(0,0,0,0.4)';
					border.style.borderColor = '#fff';
					colorEq = 'rgba(100,100,100,0.5)';
					colorGrid = 'rgba(105, 105, 105, 0.8)';
			...
			setImageBg(3, 'galaxy-image')
			setImageBg(4, 'forest-and-mount-image')
			setImageBg(5, 'london-image')
			setImageBg(6, 'air-lamp-image')
			setImageBg(7, 'mountains-image')
			setImageBg(8, 'mountains2-image')
			setImageBg(9, 'road-image')
			setImageBg(10, 'sun-image')
			setImageBg(11, 'board-image')
			setImageBg(12, 'town-image')
			setImageBg(13, 'sea-and-stars-image')
			}
		...
		document.addEventListener('click',function (e) {
			...
			if(e.target == btnView[0]){
				btnView[0].classList.toggle('view-map-elem-active');
					map.className = 'map border-white';
			...
			if(e.target == btnView[1]){
				...
				if(!map.classList.contains("view-semy-sphere")) {
					map.className = 'map view-semy-sphere border-white'
					border.classList.remove('color-white');
					mainProjection = 'fisheye'
					starmap.style.borderRadius = '50%';
					mapSettings()
			...
			if(e.target == btnView[2]){
				...
				if(!map.classList.contains("view-semy-rect")) {
					map.className = 'map view-semy-rect'
			...
			if(e.target == btnView[4]){
				btnView[4].classList.toggle('view-map-elem-active');
				if(!map.classList.contains("view-love-heart")) {
					border.classList.remove('color-white');
					map.className = 'map view-love-heart stroke-love-heart'
					starmap.style.borderRadius = '50%';
					mainProjection = 'fisheye'
			...
			if(e.target == btnView[5]){
				btnView[5].classList.toggle('view-map-elem-active');
				if(!map.classList.contains("view-heart")) {
					border.classList.remove('color-white');
					map.className = 'map view-heart stroke-heart'
					starmap.style.borderRadius = '50%';
					starmap.style.borderTopLeftRadius = '0%';
					mainProjection = 'fisheye'

					mapSettings()
				}
			}
			if(e.target == btnView[6]){
				btnView[6].classList.toggle('view-map-elem-active');
				if(!map.classList.contains("view-man")) {
					border.classList.remove('color-white');
					map.className = 'map view-man stroke-man'
					starmap.style.borderRadius = '50%';
					mainProjection = 'fisheye'
			...
			if(e.target == btnView[7]){
				btnView[7].classList.toggle('view-map-elem-active');
				if(!map.classList.contains("view-woman")) {
					border.classList.remove('color-white');
					map.className = 'map view-woman stroke-woman'
					starmap.style.borderRadius = '50%';
					mainProjection = 'fisheye'
			...
			if(e.target == btnView[8]){
				btnView[8].classList.toggle('view-map-elem-active');
				if(!map.classList.contains("view-moon")) {
					border.classList.remove('color-white');
					map.className = 'map view-moon stroke-moon'
					starmap.style.borderRadius = '50%';
					mainProjection = 'fisheye'
			...
			if(e.target == btnView[9]){
				btnView[9].classList.toggle('view-map-elem-active');
				if(!map.classList.contains("view-racket")) {
					border.classList.remove('color-white');
					map.className = 'map view-racket stroke-racket'
					starmap.style.borderRadius = '0%';
					starmap.style.borderBottomRightRadius = '50%';
					mainProjection = 'stereo'

					mapSettings()
				}
			}
			if(e.target == btnView[10]){
				btnView[10].classList.toggle('view-map-elem-active');
				if(!map.classList.contains("view-blot")) {
					border.classList.remove('color-white');
					map.className = 'map view-blot'
					starmap.style.borderRadius = '50%';
					mainProjection = 'fisheye'
			...
			if(e.target == btnView[11]){
				btnView[11].classList.toggle('view-map-elem-active');
				if(!map.classList.contains("view-saturn")) {
					border.classList.remove('color-white');
					map.className = 'map view-saturn stroke-saturn'
					starmap.style.borderRadius = '50%';
					starmap.style.borderBottomLeftRadius = '0%';
					starmap.style.borderTopRightRadius = '0%';
